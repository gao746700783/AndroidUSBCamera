...
import com.jiangdg.ausbc.R as R2
    ...
    private fun initEffectTabs() {
        getCurAnimationId().let { curAnimId ->
            if (curAnimId != CameraEffect.ID_NONE_ANIMATION) {
                ...
                mFilterTabBtn?.setTextColor(getDialog().context.resources.getColor(R2.color.common_a8_black))
        ...
        mAnimTabBtn?.setTextColor(getDialog().context.resources.getColor(R2.color.common_a8_black))
        mAnimTabBtn?.setCompoundDrawablesWithIntrinsicBounds(
            0,
            0,
            0,
    ...
    override fun convert(helper: BaseViewHolder, item: CameraEffect) {
        ...
        helper.getView<ImageView>(R.id.effectIv).also {
            item.coverResId?.apply {
                ImageLoaders.of(mContext).loadCircle(it, this, R2.drawable.effect_none)
                return@also
            }
            ImageLoaders.of(mContext).loadCircle(it, item.coverUrl, R2.drawable.effect_none)
        }
